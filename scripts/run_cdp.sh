...
SAVE_ROOT="${SAVE_ROOT:-cdp_eval_result}"
SEEDS_STR="${SEEDS:-0}"
...
for seed in "${SEED_LIST[@]}"; do
  if [[ "$RUN_OGBENCH" == "1" ]]; then
    for task_id in "${TASK_IDS[@]}"; do
      run_exp "cdp-scene-${task_id}-seed${seed}" \
        --seed="${seed}" \
      ...
      run_exp "cdp-cube-double-${task_id}-seed${seed}" \
        --seed="${seed}" \
      ...
      run_exp "cdp-cube-triple-${task_id}-seed${seed}" \
        --seed="${seed}" \
  ...
  if [[ "$RUN_D4RL" == "1" ]]; then
    run_exp "cdp-pen-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp-door-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp-hammer-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp-relocate-cloned-seed${seed}" \
      --seed="${seed}" \
  ...
  if [[ "$RUN_ONLINE" == "1" ]]; then
    run_exp "cdp-antmaze-online-seed${seed}" \
      --seed="${seed}" \
...
echo "All cdp test experiments finished."
