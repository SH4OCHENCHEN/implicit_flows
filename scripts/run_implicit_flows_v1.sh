...
for seed in "${SEED_LIST[@]}"; do
  if [[ "$RUN_OGBENCH" == "1" ]]; then
    for task_id in "${TASK_IDS[@]}"; do
      run_exp "IFv1-cube-triple-${task_id}-seed${seed}" \
        --seed="${seed}" \
        --env_name="cube-triple-play-singletask-${task_id}-v0" \
        --agent=agents/implicit_flows_v1.py \
        --agent.discount=0.995 || true

      run_exp "IFv1-cube-double-${task_id}-seed${seed}" \
        --seed="${seed}" \
        --env_name="cube-double-play-singletask-${task_id}-v0" \
        --agent=agents/implicit_flows_v1.py \
        --agent.discount=0.995 || true

      run_exp "IFv1-scene-${task_id}-seed${seed}" \
        --seed="${seed}" \
        --env_name="scene-play-singletask-${task_id}-v0" \
        --agent=agents/implicit_flows_v1.py || true
    done
  fi

  if [[ "$RUN_D4RL" == "1" ]]; then
    run_exp "IFv1-pen-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=pen-cloned-v1 \
      --agent=agents/implicit_flows_v1.py || true

    run_exp "IFv1-door-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=door-cloned-v1 \
      --agent=agents/implicit_flows_v1.py || true

    run_exp "IFv1-hammer-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=hammer-cloned-v1 \
      --agent=agents/implicit_flows_v1.py || true

    run_exp "IFv1-relocate-cloned-seed${seed}" \
      --seed="${seed}" \
  ...
  if [[ "$RUN_ONLINE" == "1" ]]; then
    run_exp "IFv1-antmaze-online-seed${seed}" \
      --seed="${seed}" \
...
echo "All implicit_flows_v1 test experiments finished."