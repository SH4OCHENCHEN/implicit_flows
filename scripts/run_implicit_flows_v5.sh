...
SAVE_ROOT="${SAVE_ROOT:-implicit_flows_v5_result}"
SEEDS_STR="${SEEDS:-0}"
...
for seed in "${SEED_LIST[@]}"; do
  if [[ "$RUN_OGBENCH" == "1" ]]; then
    for task_id in "${TASK_IDS[@]}"; do
      run_exp "implicit_flows_v5-cube-triple-${task_id}-seed${seed}" \
        --seed="${seed}" \
        --env_name="cube-triple-play-singletask-${task_id}-v0" \
        --agent=agents/implicit_flows_v5.py \
        --agent.discount=0.995 || true

      run_exp "implicit_flows_v5-puzzle4-${PUZZLE_TASK_ID}-seed${seed}" \
        --seed="${seed}" \
  ...
  if [[ "$RUN_D4RL" == "1" ]]; then
    run_exp "implicit_flows_v5-pen-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=pen-cloned-v1 \
      --agent=agents/implicit_flows_v5.py \
      --agent.q_agg=min || true

    run_exp "implicit_flows_v5-door-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "implicit_flows_v5-hammer-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=hammer-cloned-v1 \
      --agent=agents/implicit_flows_v5.py \
      --agent.q_agg=min || true

    run_exp "implicit_flows_v5-relocate-cloned-seed${seed}" \
      --seed="${seed}" \
  ...
  if [[ "$RUN_ONLINE" == "1" ]]; then
    run_exp "implicit_flows_v5-antmaze-online-seed${seed}" \
      --seed="${seed}" \
...
echo "All implicit_flows_v5 test experiments finished."
