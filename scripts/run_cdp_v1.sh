...
SAVE_ROOT="${SAVE_ROOT:-cdpv1_results}"
SEEDS_STR="${SEEDS:-0}"
...
for seed in "${SEED_LIST[@]}"; do
  if [[ "$RUN_OGBENCH" == "1" ]]; then
    for task_id in "${TASK_IDS[@]}"; do
      run_exp "cdp_v1-scene-${task_id}-seed${seed}" \
        --seed="${seed}" \
        --env_name="scene-play-singletask-${task_id}-v0" \
        --agent=agents/cdp_v1.py \
        --agent.q_agg=min \
        --agent.cql_alpha=1.0 || true

      run_exp "cdp_v1-cube-double-${task_id}-seed${seed}" \
        --seed="${seed}" \
      ...
      run_exp "cdp_v1-cube-triple-${task_id}-seed${seed}" \
        --seed="${seed}" \
  ...
  if [[ "$RUN_D4RL" == "1" ]]; then
    run_exp "cdp_v1-pen-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp_v1-door-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp_v1-hammer-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp_v1-relocate-cloned-seed${seed}" \
      --seed="${seed}" \
  ...
  if [[ "$RUN_ONLINE" == "1" ]]; then
    run_exp "cdp_v1-antmaze-online-seed${seed}" \
      --seed="${seed}" \
...
echo "All cdp_v1 test experiments finished."