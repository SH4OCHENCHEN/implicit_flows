...
SAVE_ROOT="${SAVE_ROOT:-implicit_flows_v3_result}"
SEEDS_STR="${SEEDS:-0}"
...
for seed in "${SEED_LIST[@]}"; do
  ...
  if [[ "$RUN_D4RL" == "1" ]]; then
    run_exp "implicit_flows_v3-pen-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=pen-cloned-v1 \
      --agent=agents/implicit_flows_v3.py \
      --agent.q_agg=min || true

    run_exp "implicit_flows_v3-door-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "implicit_flows_v3-hammer-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=hammer-cloned-v1 \
      --agent=agents/implicit_flows_v3.py \
      --agent.q_agg=min || true

    run_exp "implicit_flows_v3-relocate-cloned-seed${seed}" \
      --seed="${seed}" \
  ...
  if [[ "$RUN_ONLINE" == "1" ]]; then
    run_exp "implicit_flows_v3-antmaze-online-seed${seed}" \
      --seed="${seed}" \
...
echo "All implicit_flows_v3 test experiments finished."
