...
SAVE_ROOT="${SAVE_ROOT:-cdpv2_results}"
SEEDS_STR="${SEEDS:-0}"
...
for seed in "${SEED_LIST[@]}"; do
  if [[ "$RUN_OGBENCH" == "1" ]]; then
    for task_id in "${TASK_IDS[@]}"; do
      ...
      run_exp "cdp_v2-cube-triple-${task_id}-seed${seed}" \
        --seed="${seed}" \
  ...
  if [[ "$RUN_D4RL" == "1" ]]; then
    run_exp "cdp_v2-pen-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp_v2-door-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp_v2-hammer-cloned-seed${seed}" \
      --seed="${seed}" \
    ...
    run_exp "cdp_v2-relocate-cloned-seed${seed}" \
      --seed="${seed}" \
  ...
  if [[ "$RUN_ONLINE" == "1" ]]; then
    run_exp "cdp_v2-antmaze-online-seed${seed}" \
      --seed="${seed}" \
...
echo "All cdp_v2 test experiments finished."
