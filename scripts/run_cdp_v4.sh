...
SAVE_ROOT="${SAVE_ROOT:-cdpv4_results}"
SEEDS_STR="${SEEDS:-0}"
...
for seed in "${SEED_LIST[@]}"; do
  if [[ "$RUN_OGBENCH" == "1" ]]; then
    for task_id in "${TASK_IDS[@]}"; do
      # run_exp "cdp_v4-scene-${task_id}-seed${seed}" \
      #   --seed="${seed}" \
      #   --env_name="scene-play-singletask-${task_id}-v0" \
      #   --agent=agents/cdp_v4.py || true

      run_exp "cdp_v4-cube-double-${task_id}-seed${seed}" \
        --seed="${seed}" \
        --env_name="cube-double-play-singletask-${task_id}-v0" \
        --agent=agents/cdp_v4.py \
        --agent.discount=0.995 || true

      run_exp "cdp_v4-cube-triple-${task_id}-seed${seed}" \
        --seed="${seed}" \
  ...
  if [[ "$RUN_D4RL" == "1" ]]; then
    run_exp "cdp_v4-pen-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=pen-cloned-v1 \
      --agent=agents/cdp_v4.py \
      --agent.q_agg=min || true

    run_exp "cdp_v4-door-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=door-cloned-v1 \
      --agent=agents/cdp_v4.py \
      --agent.q_agg=min || true

    run_exp "cdp_v4-hammer-cloned-seed${seed}" \
      --seed="${seed}" \
      --env_name=hammer-cloned-v1 \
      --agent=agents/cdp_v4.py \
      --agent.q_agg=min || true

    run_exp "cdp_v4-relocate-cloned-seed${seed}" \
      --seed="${seed}" \
  ...
  if [[ "$RUN_ONLINE" == "1" ]]; then
    run_exp "cdp_v4-antmaze-online-seed${seed}" \
      --seed="${seed}" \
...
echo "All cdp_v4 test experiments finished."
