...
SAVE_ROOT="${SAVE_ROOT:-implicit_flows_v2_light_result}"
SEEDS_STR="${SEEDS:-1}"

# Lightweight test targets (override when needed).
CUBE_TASK_ID="${CUBE_TASK_ID:-task1}"
PUZZLE_TASK_ID="${PUZZLE_TASK_ID:-task1}"

# Lightweight runtime defaults.
...
for seed in "${SEED_LIST[@]}"; do
  run_exp "implicit_flows_v2-light-cube-triple-${CUBE_TASK_ID}-seed${seed}" \
    --seed="${seed}" \
    --env_name="cube-triple-play-singletask-${CUBE_TASK_ID}-v0" \
    --agent=agents/implicit_flows_v2.py \
    --agent.discount=0.995 || true

  run_exp "implicit_flows_v2-light-puzzle4-${PUZZLE_TASK_ID}-seed${seed}" \
    --seed="${seed}" \
...
echo "All implicit_flows_v2 lightweight tests finished."
